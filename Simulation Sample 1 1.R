...
simulate_data_from_par <- function(true_par, n, m, K, p, dates_lf, dates_hf) {
  ...
  for (t in 2:n) {
    if (
      any(is.na(gscpi_std[t, ])) ||
        any(is.na(ippi_std[t, ])) ||
        any(is.na(epi_std[t, ]))
    ) {
      ...
      gscpi_term1 <- sum(w_gscpi1 * gscpi_row)
      ippi_term1 <- sum(w_ippi1 * ippi_row)
      epi_term1 <- sum(w_epi1 * epi_row)
      gscpi_term2 <- sum(w_gscpi2 * gscpi_row)
      ippi_term2 <- sum(w_ippi2 * ippi_row)
      epi_term2 <- sum(w_epi2 * epi_row)
      gscpi_term3 <- sum(w_gscpi3 * gscpi_row)
      ippi_term3 <- sum(w_ippi3 * ippi_row)
    ...
    cpi_lf[t] <- alpha1 +
      ar_terms1 +
      midas_scales_1[1] * gscpi_term1 +
      midas_scales_1[2] * ippi_term1 +
      midas_scales_1[3] * epi_term1 +
      rnorm(1, sd = 0.15)
    ...
    ir_lf[t] <- alpha2 +
      phi_contemp2 * cpi_lf[t] +
      ar_terms2 +
      midas_scales_2[1] * gscpi_term2 +
      midas_scales_2[2] * ippi_term2 +
      midas_scales_2[3] * epi_term2 +
      rnorm(1, sd = 0.12)
    ...
    nhpi_lf[t] <- alpha3 +
      phi_contemp_cpi3 * cpi_lf[t] +
      phi_contemp_ir3 * ir_lf[t] +
      ar_terms3 +
      midas_scales_3[1] * gscpi_term3 +
      midas_scales_3[2] * ippi_term3 +
      midas_scales_3[3] * epi_term3 +
      rnorm(1, sd = 0.10)
  }
...
simulated_data <- simulate_data_from_par(
  true_par,
  n,
  m,
  K,
...
model_data_sim_1 <- na.omit(model_data)
...
fitted_by_tau_cpi <- setNames(
  lapply(tau_keys, function(k) fits_sim_1[[k]]$fitted_cpi),
  tau_keys
)
fitted_by_tau_ir <- setNames(
  lapply(tau_keys, function(k) fits_sim_1[[k]]$fitted_ir),
  tau_keys
)
fitted_by_tau_nhpi <- setNames(
  lapply(tau_keys, function(k) fits_sim_1[[k]]$fitted_nhpi),
  tau_keys
)
...
create_variable_plots <- function(params, model_data_sim_1) {
  colors <- setNames(
    c("blue", "red", "darkgreen", "purple")[seq_along(taus)],
