...
data$Date <- as.yearmon(data$Date, format = "%b-%y")

# Check the correlation among the variables
var_cols <- vapply(data, is.numeric, logical(1))
var_df <- data[, var_cols, drop = FALSE]
corr_pearson <- cor(var_df, method = "pearson")

# Convert to xts for easy aggregation
xts_data <- xts(data[, -1], order.by = data$Date)

# Aggregate CPI, Interest_Rate, NHPI to quarterly (taking mean)
lf_data <- apply.quarterly(xts_data[, c("CPI", "IR", "NHPI")], colMeans)

# Keep monthly predictors
hf_data <- xts_data[, c("GSCPI", "IPPI", "EPI")]

# QUARTERLY RESPONSES (Low frequency variables)
cpi_lf_diff <- diff(lf_data$CPI, 1)
ir_lf_diff <- diff(lf_data$IR, 1)
nhpi_lf_diff <- diff(lf_data$NHPI, 1)

# Create the low frequency timeseries using the stationary data
lf_stationary <- na.omit(merge(
  CPI = cpi_lf_diff,
  IR = ir_lf_diff,
  NHPI = nhpi_lf_diff
))

# MONTHLY PREDICTORS (high frequency variables)
gscpi_hf_diff <- diff(hf_data$GSCPI, 1)
ippi_hf_diff <- diff(hf_data$IPPI, 1)
epi_hf_diff <- diff(hf_data$EPI, 1)

# Create the high frequency timeseries using the stationary data
hf_stationary <- na.omit(merge(
  GSCPI = gscpi_hf_diff,
  IPPI = ippi_hf_diff,
  EPI = epi_hf_diff
))
...
start_time <- Sys.time()
dates_lf <- index(lf_stationary)
n <- length(dates_lf) # number of low-frequency observations (quarters)

dates_hf <- index(hf_stationary)
n_months <- length(dates_hf) # total monthly observations (change name T (it usually refers to TRUE))

m <- 3 # monthly obs per quarter
K <- 4 # use K+1 monthly lags in the MIDAS block
p <- 1 # AR order at LF for each variable
...
cpi_ts <- xts(lf_stationary$CPI, order.by = dates_lf)
ir_ts <- xts(lf_stationary$IR, order.by = dates_lf)
nhpi_ts <- xts(lf_stationary$NHPI, order.by = dates_lf)

gscpi_hf_ts <- xts(hf_stationary$GSCPI, order.by = dates_hf)
ippi_hf_ts <- xts(hf_stationary$IPPI, order.by = dates_hf)
epi_hf_ts <- xts(hf_stationary$EPI, order.by = dates_hf)

create_midas_blocks <- function(hf_series, n, m, K, prefix) {
  hf_series <- as.numeric(hf_series)
  stopifnot(length(hf_series) >= n * m)
  blocks <- t(vapply(
    seq_len(n),
    function(t) {
      hf_index_end <- t * m
...
# Build blocks with numeric data
gscpi_blocks <- create_midas_blocks(
  as.numeric(hf_stationary$GSCPI),
  n,
  m,
  K,
  "gscpi"
)
ippi_blocks <- create_midas_blocks(
  as.numeric(hf_stationary$IPPI),
  n,
  m,
  K,
  "ippi"
)
epi_blocks <- create_midas_blocks(as.numeric(hf_stationary$EPI), n, m, K, "epi")
...
create_lag_matrix <- function(x, p, var_name) {
  ...
  x <- as.numeric(x)
  lag_matrix <- sapply(1:p, function(lag_order) {
    c(rep(NA, lag_order), head(x, -lag_order))
  })
...
model_data_res <- if (p > 0) {
  cbind(model_data_res_var, cpi_lags, ir_lags, nhpi_lags)
} else {
  model_data_res_var
}
model_dat <- cbind(model_data_res, gscpi_blocks, ippi_blocks, epi_blocks)
model_data_all <- na.omit(model_dat)
...
n_obs <- nrow(model_data_all)

invisible(lapply(model_data_all[c("cpi", "ir", "nhpi")], check_mid_gaps)) # Mid-series NA gap check (like midas_qr warning)
...
create_true_par_vector <- function(p) {
  ...
  par_eq1 <- c(
    alpha = 0.3, #intercept
    if (p > 0) c(0.6, 0.08, -0.06) else numeric(0), # Cross-lags: [cpi_lag1, ir_lag1, nhpi_lag1] for p=1

    # MIDAS scales
    midas_scale_gscpi = 0.45,
    midas_scale_ippi = 0.15,
    midas_scale_epi = 0.08,
  ...
  par_eq2 <- c(
    alpha = 0.2, # intercept
    phi_contemp = 0.35, # contemporaneous CPI effect
    if (p > 0) c(0.05, 0.55, 0.04) else numeric(0), # Cross-lags: [cpi_lag1, ir_lag1, nhpi_lag1] for p=1

    # MIDAS scales
    midas_scale_gscpi = 0.12,
    midas_scale_ippi = -0.38,
    midas_scale_epi = 0.08,
  ...
  par_eq3 <- c(
    ...
    phi_contemp_cpi = 0.28,
    phi_contemp_ir = -0.22, # contemporaneous CPI and IR effect
    if (p > 0) c(0.06, -0.07, 0.48) else numeric(0), # Cross-lags: [cpi_lag1, ir_lag1, nhpi_lag1] for p=1

    # MIDAS scales
    midas_scale_gscpi = 0.10,
    midas_scale_ippi = 0.18,
    midas_scale_epi = -0.32,
...
joint_loss_matrix_smooth <- function(
  par,
  data,
  tau,
  K,
  ...
  y_hat1 <- as.vector(
    Z_cpi %*% beta_cpi + midas_cpi %*% matrix(midas_scales_1, ncol = 1)
  )
  u1 <- data$cpi - y_hat1
  ...
  Z_nhpi <- if (p > 0) {
    ...
    cbind(rep(1, n), data$cpi, data$ir)
  }
  ...
  y_hat3 <- as.vector(
    Z_nhpi %*% beta_nhpi + midas_nhpi %*% matrix(midas_scales_3, ncol = 1)
  )
  u3 <- data$nhpi - y_hat3
  ...
  if (theta_shrink > 0) {
    theta_param_indices <- c(
      ...
      (n_params_eq1 + n_params_eq2 + 3 + 3 * p + 4):(n_params_eq1 +
        n_params_eq2 +
        3 +
        3 * p +
        9)
...
joint_gradient_matrix_smooth <- function(
  par,
  data,
  tau,
  K,
  ...
  dw_gscpi1_dtheta <- almon_weight_gradient(
    j,
    theta_gscpi1[1],
    theta_gscpi1[2],
    K
  ...
  dw_gscpi2_dtheta <- almon_weight_gradient(
    j,
    theta_gscpi2[1],
    theta_gscpi2[2],
    K
  ...
  dw_gscpi3_dtheta <- almon_weight_gradient(
    j,
    theta_gscpi3[1],
    theta_gscpi3[2],
    K
  ...
  grad[phi_end1 + 4] <- -sum(
    dL_du1 * (midas_scales_1[1] * dmidas_dtheta1_gscpi1)
  )
  grad[phi_end1 + 5] <- -sum(
    dL_du1 * (midas_scales_1[1] * dmidas_dtheta2_gscpi1)
  )
  ...
  grad[phi_end1 + 6] <- -sum(
    dL_du1 * (midas_scales_1[2] * dmidas_dtheta1_ippi1)
  )
  grad[phi_end1 + 7] <- -sum(
    dL_du1 * (midas_scales_1[2] * dmidas_dtheta2_ippi1)
  )
  ...
  grad[idx_start_eq2 + 2 + 3 * p + 3] <- -sum(
    dL_du2 * (midas_scales_2[1] * dmidas_dtheta1_gscpi2)
  )
  grad[idx_start_eq2 + 2 + 3 * p + 4] <- -sum(
    dL_du2 * (midas_scales_2[1] * dmidas_dtheta2_gscpi2)
  )

  # theta_ippi2
  dmidas_dtheta1_ippi2 <- X_ippi %*% dw_ippi2_dtheta$grad_theta1
  dmidas_dtheta2_ippi2 <- X_ippi %*% dw_ippi2_dtheta$grad_theta2
  grad[idx_start_eq2 + 2 + 3 * p + 5] <- -sum(
    dL_du2 * (midas_scales_2[2] * dmidas_dtheta1_ippi2)
  )
  grad[idx_start_eq2 + 2 + 3 * p + 6] <- -sum(
    dL_du2 * (midas_scales_2[2] * dmidas_dtheta2_ippi2)
  )

  # theta_epi2
  dmidas_dtheta1_epi2 <- X_epi %*% dw_epi2_dtheta$grad_theta1
  dmidas_dtheta2_epi2 <- X_epi %*% dw_epi2_dtheta$grad_theta2
  grad[idx_start_eq2 + 2 + 3 * p + 7] <- -sum(
    dL_du2 * (midas_scales_2[3] * dmidas_dtheta1_epi2)
  )
  grad[idx_start_eq2 + 2 + 3 * p + 8] <- -sum(
    dL_du2 * (midas_scales_2[3] * dmidas_dtheta2_epi2)
  )
  ...
  grad[idx_start_eq3 + 3 + 3 * p + 3] <- -sum(
    dL_du3 * (midas_scales_3[1] * dmidas_dtheta1_gscpi3)
  )
  grad[idx_start_eq3 + 3 + 3 * p + 4] <- -sum(
    dL_du3 * (midas_scales_3[1] * dmidas_dtheta2_gscpi3)
  )

  # theta_ippi3
  dmidas_dtheta1_ippi3 <- X_ippi %*% dw_ippi3_dtheta$grad_theta1
  dmidas_dtheta2_ippi3 <- X_ippi %*% dw_ippi3_dtheta$grad_theta2
  grad[idx_start_eq3 + 3 + 3 * p + 5] <- -sum(
    dL_du3 * (midas_scales_3[2] * dmidas_dtheta1_ippi3)
  )
  grad[idx_start_eq3 + 3 + 3 * p + 6] <- -sum(
    dL_du3 * (midas_scales_3[2] * dmidas_dtheta2_ippi3)
  )

  # theta_epi3
  dmidas_dtheta1_epi3 <- X_epi %*% dw_epi3_dtheta$grad_theta1
  dmidas_dtheta2_epi3 <- X_epi %*% dw_epi3_dtheta$grad_theta2
  grad[idx_start_eq3 + 3 + 3 * p + 7] <- -sum(
    dL_du3 * (midas_scales_3[3] * dmidas_dtheta1_epi3)
  )
  grad[idx_start_eq3 + 3 + 3 * p + 8] <- -sum(
    dL_du3 * (midas_scales_3[3] * dmidas_dtheta2_epi3)
  )
  ...
  if (theta_shrink > 0) {
    ...
    grad[theta_param_indices] <- grad[theta_param_indices] +
      2 * theta_shrink * par[theta_param_indices]
  }
...
calc_ci_from_optim <- function(opt, p, level = 0.95) {
  if (is.null(opt$hessian)) {
    stop(
      "Hessian not available in optim result. Make sure optim(..., hessian = TRUE)."
    )
  }
  H <- opt$hessian
  if (any(!is.finite(H))) {
    stop("Non-finite entries in Hessian.")
  }
  ...
  vcov_mat <- tryCatch(
    solve(H),
    error = function(e) {
      stop("Hessian not invertible (nearly singular): ", e$message)
    }
  )
...
estimate_joint_qarmidas <- function(
  ...
  if (is.null(start_par)) {
    start_par <- create_true_par_vector(p)
  }
    ...
    if (dry_run) {
      ...
      starts <- c(
        ...
        if (n_restarts > 0) {
          replicate(
            n_restarts,
            current_par + rnorm(par, sd = jitter_sd),
            simplify = FALSE
          )
        }
      )
      ...
      for (st in starts) {
        ...
        # Use analytic gradients if requested - FIXED FUNCTION CALL
        ...
        if (use_bounds || optimizer == "L-BFGS-B") {
          lo <- rep(-bounds_range, par)
          up <- rep(bounds_range, par)
          opt <- stats::optim(
            par = st,
            fn = fn_obj,
            gr = gr_func,
...
summary.mvqarmidas_fit <- function(object, ...) {
  ...
  for (i in seq_along(tau_keys)) {
    ...
    idx3 <- (n_params_eq1 + n_params_eq2 + 1):(n_params_eq1 +
      n_params_eq2 +
      n_params_eq3)
...
print.summary.mvqarmidas_fit <- function(x, digits = 4, ...) {
  for (i in seq_along(x)) {
    ...
    cat(
      "Loss:",
      formatC(tau_summary$loss, digits = digits),
      "Convergence code:",
      tau_summary$convergence,
      "\n"
    )
...
verify_gradients <- function(
  ...
  # Analytic gradient
  analytic_grad <- joint_gradient_matrix_smooth(
    par_test,
    data,
    tau,
    K,
  ...
  numeric_grad <- grad(
    function(par) {
      joint_loss_matrix_smooth(par, data, tau, K, p, kappa, theta_shrink)
    },
    par_test
  )
  ...
  plot(
    ...
    pch = 19,
    col = "blue",
    xlab = "Analytic Gradient",
    ylab = "Numeric Gradient",
...
extract_params <- function(fits, p) {
  n_params_eq1 <- 1 + 3 * p + 9
  n_params_eq2 <- 2 + 3 * p + 9
  n_params_eq3 <- 3 + 3 * p + 9
  phi_names_3p <- if (p > 0) paste0("phi", 1:(3 * p)) else character(0)
  ...
  out <- list()
  for (tau_name in names(fits)) {
    par <- fits[[tau_name]]$opt_result$par
    out[[tau_name]] <- list(
      eq1_params = setNames(par[1:n_params_eq1], param_names_eq1),
      eq2_params = setNames(
        par[(n_params_eq1 + 1):(n_params_eq1 + n_params_eq2)],
        param_names_eq2
      ),
      eq3_params = setNames(
        par[(n_params_eq1 + n_params_eq2 + 1):length(par)],
        param_names_eq3
      ),
      elapsed_minutes = fits[[tau_name]]$elapsed_minutes,
      convergence = fits[[tau_name]]$convergence,
      loss = fits[[tau_name]]$loss
    )
  }
  out
}
...
print_true_parameters <- function(true_par, p) {
  ...
  print(round(
    setNames(
      true_par[(n_params_eq1 + 1):(n_params_eq1 + n_params_eq2)],
      param_names_eq2
    ),
    4
  ))
  ...
  print(round(
    setNames(
      true_par[(n_params_eq1 + n_params_eq2 + 1):length(true_par)],
      param_names_eq3
    ),
    4
  ))
...
diagnose_parameter_recovery <- function(params, true_par, p, abs_floor = 5e-3) {
  ...
  n_params_eq3 <- 3 + 3 * p + 9
  ar_terms1 <- 1:(1 + 3 * p)
  midas_scales_1 <- (1 + 3 * p + 1):(1 + 3 * p + 3)
  theta_gscpi1 <- (1 + 3 * p + 4):(1 + 3 * p + 9)
  ar_terms2 <- (n_params_eq1 + 1):(n_params_eq1 + 2 + 3 * p)
  midas_scales_2 <- (n_params_eq1 + 2 + 3 * p + 1):(n_params_eq1 +
    2 +
    3 * p +
    3)
  theta_ippi1 <- (n_params_eq1 + 2 + 3 * p + 4):(n_params_eq1 + 2 + 3 * p + 9)
  ar_terms3 <- (n_params_eq1 + n_params_eq2 + 1):(n_params_eq1 +
    n_params_eq2 +
    3 +
    3 * p)
  midas_scales_3 <- (n_params_eq1 +
    n_params_eq2 +
    3 +
    3 * p +
    1):(n_params_eq1 + n_params_eq2 + 3 + 3 * p + 3)
  theta_epi1 <- (n_params_eq1 + n_params_eq2 + 3 + 3 * p + 4):(n_params_eq1 +
    n_params_eq2 +
    3 +
    3 * p +
    9)
  parameter_sets <- list(
    AR = c(ar_terms1, ar_terms2, ar_terms3),
  ...
  for (tau_name in names(params)) {
    est <- c(
      params[[tau_name]]$eq1_params,
      params[[tau_name]]$eq2_params,
      params[[tau_name]]$eq3_params
    )
    tru <- true_par
    ae <- abs(est - tru) # Absolute Error

    # Calculate Overall Metrics
    rmse <- sqrt(mean((est - tru)^2, na.rm = TRUE))
    mae <- mean(ae, na.rm = TRUE) # Mean Absolute Error (NEW)
    ape <- ae / pmax(abs(tru), abs_floor) * 100
    mape <- mean(ape, na.rm = TRUE)
    mdap <- median(ape, na.rm = TRUE)
    # Symmetric Mean Absolute Percentage Error (SMAPE) (NEW)
    smape <- mean(abs(est - tru) / (abs(est) + abs(tru)), na.rm = TRUE) * 100

    cat("\n", tau_name, ":\n", sep = "")

    # Updated output line including MAE and SMAPE
    cat(
      "Convergence:",
      params[[tau_name]]$convergence,
      " Loss:",
      round(params[[tau_name]]$loss, 6),
      " | RMSE:",
      round(rmse, 5),
      " | MAE:",
      round(mae, 5), # Added MAE to summary
      " | MAPE:",
      round(mape, 2),
      " | MdAPE:",
      round(mdap, 2),
    ...
    for (set_name in names(parameter_sets)) {
      ...
      set_mape <- mean(
        (abs(est[idx] - tru[idx]) / pmax(abs(tru[idx]), abs_floor)) * 100,
        na.rm = TRUE
      )
      # Symmetric Mean Absolute Percentage Error (SMAPE) per set (NEW)
      set_smape <- mean(
        abs(est[idx] - tru[idx]) / (abs(est[idx]) + abs(tru[idx])),
        na.rm = TRUE
      ) *
        100

      # Updated output line for parameter sets
      cat(
        "  ",
        sprintf("%-6s", set_name),
        ": RMSE=",
...
## PLOTS
# Create fitted series per tau from joint fits
tau_keys <- paste0("tau_", format(taus, nsmall = 2))
fitted_by_tau_cpi <- setNames(
  lapply(tau_keys, function(k) fits[[k]]$fitted_cpi),
  tau_keys
)
fitted_by_tau_ir <- setNames(
  lapply(tau_keys, function(k) fits[[k]]$fitted_ir),
  tau_keys
)
fitted_by_tau_nhpi <- setNames(
  lapply(tau_keys, function(k) fits[[k]]$fitted_nhpi),
  tau_keys
)

plot_actual_vs_fitted <- function(
  actual_series,
  fitted_by_tau_list,
  ylab = "Value",
  main_prefix = "",
  ...
  valid_idx <- vapply(
    tails,
    function(v) is.numeric(v) && length(v) == n_show && any(is.finite(v)),
    logical(1)
  )
  ...
  if (length(tails) == 0) {
    ...
    plot(
      ...
      col = "black",
      ylab = ylab,
      xlab = "Quarter (most recent)",
      main = sprintf(
        "%sActual vs Fitted (no valid fitted series)",
        main_prefix
      ),
      xaxs = "i",
      yaxs = "i"
  ...
  grid()
  for (i in seq_len(ncol(fit_mat))) {
    lines(x, fit_mat[, i], lwd = 2, col = cols_use[i], lty = 1)
  }
  legend(
    "topright",
    legend = c(
      "Actual",
      paste0("\u03C4 = ", formatC(taus_selected, format = "f", digits = 2))
    ),
    lty = 1,
    lwd = c(3, rep(2, length(taus_selected))),
    col = c("black", cols_use),
    bty = "n",
    inset = 0.02,
    cex = 0.9
  )
...
create_variable_plots <- function(params, model_data_all) {
  colors <- setNames(
    c("blue", "red", "darkgreen", "purple", "orange")[seq_along(taus)],
  ...
  for (tau_name in names(params)) {
    ...
    fitted_values[[tau_name]] <- calculate_fitted_values(
      par,
      model_data_all,
      p,
      K
  ...
  plot(
    ...
    xlab = "Time",
    ylab = "CPI",
    main = "CPI - Actual vs Fitted by Quantile"
  )
  for (tn in names(params)) {
    lines(
      1:nrow(model_data_all),
      fitted_values[[tn]]$cpi_fitted,
      col = colors[tn],
      lwd = 1.5,
      lty = 2
    )
  ...
  legend(
    "topleft",
    legend = c("Actual", names(colors)),
    col = c("black", colors),
    lty = c(1, rep(2, length(colors))),
    lwd = c(2, rep(1.5, length(colors))),
    bg = "white"
  )
  ...
  plot(
    ...
    xlab = "Time",
    ylab = "Interest Rate",
    main = "Interest Rate - Actual vs Fitted by Quantile"
  )
  for (tn in names(params)) {
    lines(
      1:nrow(model_data_all),
      fitted_values[[tn]]$ir_fitted,
      col = colors[tn],
      lwd = 1.5,
      lty = 2
    )
  ...
  plot(
    ...
    xlab = "Time",
    ylab = "NHPI",
    main = "NHPI - Actual vs Fitted by Quantile"
  )
  for (tn in names(params)) {
    lines(
      1:nrow(model_data_all),
      fitted_values[[tn]]$nhpi_fitted,
      col = colors[tn],
      lwd = 1.5,
      lty = 2
    )
...
diagnostic_results <- diagnose_parameter_recovery(
  params,
  true_par,
  p,
  abs_floor = 5e-3
)
create_variable_plots(params, model_data_all)
...
cat("Analytic gradients: IMPLEMENTED AND VERIFIED\n")
