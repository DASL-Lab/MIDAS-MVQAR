...
create_midas_blocks <- function(hf_series, n, m, K, prefix) {
  ...
  blocks <- t(vapply(seq_len(n), function(t) {
    hf_index_end   <- t * m                  
...
gscpi_blocks <- create_midas_blocks(as.numeric(hf_stationary$GSCPI), n, m, K, "gscpi")
ippi_blocks  <- create_midas_blocks(as.numeric(hf_stationary$IPPI),  n, m, K, "ippi")
...
stopifnot(all(vapply(model_data_all, function(col) all(is.finite(col)), logical(1))))
...
create_true_par_vector <- function(p) {
  ...
  par_eq1 <- c(
    ...
    theta1_gscpi = -0.25, theta2_gscpi = 0.035,
    theta1_ippi  = -0.12, theta2_ippi  = 0.018,
    theta1_epi   = -0.18, theta2_epi   = 0.025
  )
  ...
  par_eq2 <- c(
    ...
    # theta parameters for each predictor
    theta1_gscpi = 0.15, theta2_gscpi = -0.022,
    theta1_ippi  = 0.22, theta2_ippi  = -0.032,
    theta1_epi   = 0.10, theta2_epi   = -0.015
  )
  
  
  par_eq3 <- c(
    alpha = 0.25,                                      #intercept
    phi_contemp_cpi = 0.28, phi_contemp_ir = -0.22,     # contemporaneous CPI and IR effect
    ...
    # theta parameters for each predictor
    theta1_gscpi = 0.20, theta2_gscpi = -0.028,
    theta1_ippi  = 0.14, theta2_ippi  = -0.020,
    theta1_epi   = 0.25, theta2_epi   = -0.038
  )
...
estimate_joint_qarmidas <- function(
  ...
  for (tau in sort(taus)) {
    ...
    if (verbose && !dry_run)
      cat(" Done t =", tau, "conv =", best$convergence, "loss =", round(best$value, 6), "\n")
...
diagnose_parameter_recovery <- function(params, true_par, p, abs_floor = 5e-3) {
  ...
  for (tau_name in names(params)) {
    ...
    cat("Convergence:", params[[tau_name]]$convergence,
        ...
        " | RMSE:", round(rmse, 5),
        " | MAE:", round(mae, 5), # Added MAE to summary
        " | MAPE:", round(mape, 2),
        " | MdAPE:", round(mdap, 2),
    ...
    for (set_name in names(parameter_sets)) {
      ...
      set_smape <- mean(abs(est[idx] - tru[idx]) / (abs(est[idx]) + abs(tru[idx])), na.rm = TRUE) * 100
      
      # Updated output line for parameter sets
      cat("  ", sprintf("%-6s", set_name), ": RMSE=", round(set_rmse, 5),
          ", MAE=", round(set_mae, 5),
          ", MAPE=", round(set_mape, 2),
          ", SMAPE=", round(set_smape, 2), "\n", sep = "") # Added SMAPE to set summary
    }
  }
}
...
fits <- estimate_joint_qarmidas(
  taus = taus, data = model_data_all, K = K, p = p,
  kappa = 0.01,
  theta_shrink = 2e-3,     
  start_par = true_par,
  optimizer = "BFGS",
...
tau_keys <- paste0("tau_", format(taus, nsmall = 2))
fitted_by_tau_cpi  <- setNames(lapply(tau_keys, function(k) fits[[k]]$fitted_cpi), tau_keys)
fitted_by_tau_ir   <- setNames(lapply(tau_keys, function(k) fits[[k]]$fitted_ir),  tau_keys)
fitted_by_tau_nhpi <- setNames(lapply(tau_keys, function(k) fits[[k]]$fitted_nhpi),tau_keys)
...
plot_actual_vs_fitted(model_data_all$cpi,  fitted_by_tau_cpi,  ylab = "CPI",           main_prefix = "CPI - ")
plot_actual_vs_fitted(model_data_all$ir,   fitted_by_tau_ir,   ylab = "Interest Rate", main_prefix = "Interest Rate - ")
plot_actual_vs_fitted(model_data_all$nhpi, fitted_by_tau_nhpi, ylab = "NHPI",          main_prefix = "NHPI - ")
par(mfrow = c(1, 1))

create_variable_plots <- function(params, model_data_all) {
  colors <- setNames(c("blue","red","darkgreen","purple", "orange")[seq_along(taus)],
  ...
  for (tau_name in names(params)) {
    ...
    fitted_values[[tau_name]] <- calculate_fitted_values(par, model_data_all, p, K)
  }
  plot(1:nrow(model_data_all), model_data_all$cpi, type = "l", lwd = 2, col = "black",
       xlab = "Time", ylab = "CPI", main = "CPI - Actual vs Fitted by Quantile")
  for (tn in names(params)) lines(1:nrow(model_data_all), fitted_values[[tn]]$cpi_fitted, col = colors[tn], lwd = 1.5, lty = 2)
  legend("topleft", legend = c("Actual", names(colors)), 
         col = c("black", colors), lty = c(1, rep(2, length(colors))), lwd = c(2, rep(1.5, length(colors))), bg = "white")
  
  plot(1:nrow(model_data_all), model_data_all$ir, type = "l", lwd = 2, col = "black",
       xlab = "Time", ylab = "Interest Rate", main = "Interest Rate - Actual vs Fitted by Quantile")
  for (tn in names(params)) lines(1:nrow(model_data_all), fitted_values[[tn]]$ir_fitted, col = colors[tn], lwd = 1.5, lty = 2)
  legend("topleft", legend = c("Actual", names(colors)), 
         col = c("black", colors), lty = c(1, rep(2, length(colors))), lwd = c(2, rep(1.5, length(colors))), bg = "white")
  
  plot(1:nrow(model_data_all), model_data_all$nhpi, type = "l", lwd = 2, col = "black",
       xlab = "Time", ylab = "NHPI", main = "NHPI - Actual vs Fitted by Quantile")
  for (tn in names(params)) lines(1:nrow(model_data_all), fitted_values[[tn]]$nhpi_fitted, col = colors[tn], lwd = 1.5, lty = 2)
  legend("topleft", legend = c("Actual", names(colors)), 
         col = c("black", colors), lty = c(1, rep(2, length(colors))), lwd = c(2, rep(1.5, length(colors))), bg = "white")
}
